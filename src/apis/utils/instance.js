 ...
 *
 * 직관적인 에러처리
 * 서버의 API를 찾을 수 없거나(404), 서버 내부에 문제(500)가 생겼을때 fetch() 는 이를 성공적인 요청으로 간주하지만 axios는 서버 에러를 실패로 처리해서 try...catch구문으로 쉽게 잡아낼 수 있다.
 * 요청/응답 가로채기 (Interceptors)
 * 모든 요청이 보내지기 전이나 모든 응답이 도착한 후에 중간 지점에서 공통 작업을 처리할 수 있다.
